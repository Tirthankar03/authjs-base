// useSessionDataData.ts (replace useSessionData hook)
'use client'
 ...
 * @throws {Error} - If `useSessionDataData` is not wrapped in a <SessionDataProvider /> where the error message is shown only in development mode.
 *
 * @returns {TSessionContextValue} - The session data obtained from the SessionContext.
 */
export function useSessionDataData(): TSessionContextValue {
	if (!SessionContext) {
	...
	if (!sessionContent && process.env.NODE_ENV !== 'production') {
		throw new Error('[auth-wrapper-error]: `useSessionDataData` must be wrapped in a <SessionDataProvider />')
	}
